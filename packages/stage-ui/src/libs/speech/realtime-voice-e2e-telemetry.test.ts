...
describe('realtime voice E2E telemetry', () => {
  ...
  it('correlates one voice turn and attributes the 1200 ms transcript buffer delay', () => {
    const turnId = createRealtimeVoiceTurn({ turnId: 'voice-turn-1', at: 100 })
  ...
  it('preserves signed, zero, and sub-millisecond intervals without clamping', () => {
    const turnId = createRealtimeVoiceTurn({ turnId: 'voice-turn-2', at: 100.25 })
  ...
  it('requires completion milestones, ignores duplicate completion, and isolates turns', () => {
    const firstTurnId = createRealtimeVoiceTurn({ turnId: 'voice-turn-first', at: 1 })
    ...
    const secondTurnId = createRealtimeVoiceTurn({ turnId: 'voice-turn-second', at: 10 })
  ...
  it('does not produce a successful report after cancel or failure', () => {
    const cancelledTurnId = createRealtimeVoiceTurn({ turnId: 'voice-cancelled', at: 1 })
    cancelRealtimeVoiceTurn(cancelledTurnId)
    expect(completeRealtimeVoiceTurn(cancelledTurnId)).toBeUndefined()

    const failedTurnId = createRealtimeVoiceTurn({ turnId: 'voice-failed', at: 1 })
  ...
  it('ignores non-finite timestamps and keeps the payload content-free', () => {
    const turnId = createRealtimeVoiceTurn({ turnId: 'voice-safe', at: Number.NaN })
