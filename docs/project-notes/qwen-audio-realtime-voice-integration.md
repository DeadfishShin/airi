...
当前 VAD 诊断沿用生产配置值：threshold `0.52`、`minSilenceDurationMs=1200`、`speechPadMs=360`、`minSpeechDurationMs=300`。生产 VAD 的模型身份现已固定为 `onnx-community/silero-vad`、revision `ddc9a7e80d6758f6fc795a1e8a04b798eb929d3a`、`onnx/model.onnx`、`fp32`；vendored artifact 的 SHA-256 为 `a4a068cd6cf1ea8355b84327595838ca748ec29a25bc91fc82e6c299ccdc5808`，大小为 `2243022` bytes，许可证为 MIT。已用禁止远程模型且拒绝 `fetch` 的测试实例化实际 AIRI `workers/vad/VAD`，对合成数值输入完成有限概率输出，证明了生产类、同一 artifact 和推理契约可以离线加载；这不是语音质量或 AEC 结论。`SOURCE_PROVEN`。

现在的 diagnostic renderer 已移除 `@ricky0123/vad-web`、其 Silero v5 compatibility bundle 和 `ScriptProcessor` 路径，改为使用 AIRI production `workers/vad/VAD`、共享的 `resolveProductionVADConfig`、`createVADStates` 与 production `process.worklet`。Transformers 运行时固定 `allowRemoteModels=false`、`allowLocalModels=true`，模型只从仓库内 vendored 的 immutable `onnx-community/silero-vad` revision/asset 通过 diagnostic Electron session 的受限本地协议提供；这使 `PRODUCTION_VAD_ALIGNMENT=YES` 的含义严格限定为 detector、model、config 和 AudioWorklet graph 已对齐 production authority，不代表 AEC Level 2/3 或 barge-in 已通过。`SOURCE_PROVEN`。

该历史 standalone Electron smoke 保留 `getUserMedia` 的 `echoCancellation`、`noiseSuppression`、`autoGainControl` 请求和 PHASE_0–4 协议，且使用 production VAD 配置：threshold `0.52`、exit threshold `0.156`、`minSilenceDurationMs=1200`、`speechPadMs=360`、`minSpeechDurationMs=300`、sample rate `16000`。Level 2 仍只接受 `MediaStreamTrack.getSettings().echoCancellation === true`；Level 3 还必须在可解释的静音 baseline、隔离的 phase、代表性 playback 和两类用户讲话控制下成立。该历史路径使用的 `synthetic-compatibility` playback 不能单独授予 full Level-3 PASS；Owner 尚未在该历史路径运行 PHASE_0–4，runtime verdict 仍为 `OPEN`。当前 Chromium Owner 路线已在后续章节改用真实本地 speech waveform。`SOURCE_PROVEN` + `CURRENT_DESIGN_DECISION`。
...
Owner 首次运行暴露了另一个独立的启动阻塞：standalone smoke 的 programmatic Vite server 以 repository root 为 `root`，而 Vite 8.2.2 在没有显式 `optimizeDeps.entries` 时会按 `**/*.html` 推导扫描入口。仓库中的 `plugins/airi-plugin-web-extension/entrypoints/popup/index.html` 因此被扫描，其 `main.ts` 导入 `uno.css`，产生了 Owner 看到的 dependency-scan error。对照启动证明该 error 可能使依赖预打包被跳过而不必让 `listen()` 抛错；所以 `uno.css` 不是 smoke renderer 的依赖，也不能单独被写成 renderer bootstrap failure。已证明的 causal seam 是 smoke Vite 实例把无关 workspace HTML 纳入启动依赖发现；这会污染 standalone smoke 的启动边界，Owner 的 UI 未初始化则以实际启动结果为准，不能仅由 warning 推断。`SOURCE_PROVEN` + Owner startup evidence。

此前 standalone launcher 的 Vite `configFile: false` / `optimizeDeps.disabled: true` 修复属于历史启动修复，不再是 Owner 的最终测试 host。当前 `smoke:realtime-voice-local-duplex` 通过显式 `AIRI_LOCAL_DUPLEX_DIAGNOSTIC_MODE=interactive` 进入实际 `stage-tamagotchi` production Electron main entry；main process 创建诊断窗口、绑定隔离 session，并跳过正常 provider/chat bootstrap。`smoke:realtime-voice-local-duplex:preflight` 会真正构建 diagnostic renderer、启动 production Electron host、等待 renderer→main readiness handshake，再在无媒体请求前退出。无媒体 host preflight 已通过；这只证明 production-host/module readiness，不是 Owner PHASE_0–4，也不是 Level 2/3 runtime PASS。Owner 仍需重新运行原有单一命令完成本机验证。`SOURCE_PROVEN` + `CURRENT_DESIGN_DECISION`。

两次历史 Owner 运行 standalone Electron launcher 均未进入 PHASE_0，因此其结果不是 Level 2/3 FAIL；该 launcher 也不再作为 Owner final test authority。新的 no-media production-host probe 只验证 `HOST_RUNTIME=STAGE_TAMAGOTCHI_PRODUCTION_ELECTRON`、`WINDOW_CREATED=YES`、`DIAGNOSTIC_RENDERER_LOADED=YES`、`READY_FOR_OWNER_PHASE0=YES`、`MEDIA_REQUESTED=NO` 与 `EXTERNAL_NETWORK_REQUEST_COUNT=0`。`LEVEL2=OPEN`、`LEVEL3=OPEN`，等待 Owner 在本机实际运行五阶段协议。`SOURCE_PROVEN` + `OWNER_RUNTIME_EVIDENCE`。

### Production-host VAD initialization 的 local-only network guard 修复

Owner 随后已真实完成 production Electron host、diagnostic renderer 与 PHASE_0 track inspection；`getSettings().echoCancellation === true`，因此 `AEC_SETTING=YES`、`AEC_LEVEL_2_PASS=YES`，当次还报告 `sampleRate=48000`、`channelCount=1`。production VAD 初始化随后被 local-only guard 阻止，PHASE_1–4 没有开始；这不是 host 启动失败，也不是 Level 2 FAIL，Level 3 仍为 `NOT_EXECUTED / OPEN`。该 Owner evidence 属于 `OWNER_RUNTIME_EVIDENCE`。

因果链已由源码与无媒体复现收口：安装的 `@huggingface/transformers@3.8.1` ONNX backend 在 `wasmPaths` 未设置时默认使用 `https://cdn.jsdelivr.net/.../dist/`。旧的 production-host diagnostic renderer 没有显式绑定 wasm asset，故 browser VAD 初始化会触发被 guard 拒绝的外部 ONNX wasm request；请求分类现在只保留 scheme、hostname、闭合集合的 resource class 和 resource type，不记录 query、headers、credentials 或内容。renderer 原始 `FAILURE_CODE` 与 `NETWORK_GUARD_FAILURE` 分开保留，不能用 `external-network-blocked` 覆盖原始原因。`SOURCE_PROVEN`。
...
历史上该 Chromium 路线曾使用安全保守的全本地 `synthetic-compatibility`，因此当时不能授予 full Level-3。本轮修复将 Chromium interactive path 改为 `/usr/bin/say` 生成的固定、临时本地 speech waveform，并通过同一 Chromium/WebAudio output path 播放；若系统 speech generator 或本地 asset 不可用，harness fail closed，不回退为可授予 Level-3 的兼容信号。no-media preflight 只验证 asset availability、decode 和 playback graph，不授予 full Level-3。Electron Level-2 仍是已接受的 `PASS`（保留上述 runtime evidence，不要求 Owner 重做），Chromium Level-3 与 automatic barge-in 仍未执行；完成后必须停止 tracks、销毁 VAD、停止 playback、关闭/挂起 `AudioContext`、清理 timers/listeners 和临时浏览器 profile。`OPEN / NOT_YET_PROVEN`。
...
### Chromium Level-3 真实语音播放与 VAD probability observability

Owner 已真实完成一次完整的 Chromium PHASE_1–4：quiet baseline 与 playback-only 均无 credible speech-start，环境被判定为可解释；但 user-only 与 user-during-playback 均没有 production VAD speech-start。因此当次结果是 `INCONCLUSIVE_USER_SPEECH_NOT_DETECTED`，不是 AEC FAIL；Owner 补充的少量环境音只记录为 `minor-background-noise`，不能覆盖已通过的 quiet baseline。终端的 pnpm Exit 1 是 candidate gate 未通过后的预期非零退出，不是新的 build/runtime crash。该证据属于 `OWNER_RUNTIME_EVIDENCE`。

这次修复为现有 production `VAD` 的既有 `debug` probability event 增加 bounded scalar aggregates：每个 phase 记录 debug callback/inference 计数、finite probability sample count、maximum/mean probability、超过 speech threshold `0.52` 的次数、超过 exit threshold `0.156` 的次数，并派生有限枚举 `NO_VAD_FRAMES`、`VAD_FRAMES_PRESENT_BUT_LOW_PROBABILITY`、`PROBABILITY_ABOVE_THRESHOLD_BUT_NO_SPEECH_START`、`USER_SPEECH_DETECTED`。只记录标量，不记录 PCM、waveform、transcript 或语音内容；没有改变模型、AudioWorklet、阈值或 state-machine。下一次 Owner run 可据此区分“没有实时帧”“帧已进入但概率过低”“概率越阈但事件门控未触发”和“已检测到 speech-start”。`SOURCE_PROVEN` + `CURRENT_DESIGN_DECISION`。
