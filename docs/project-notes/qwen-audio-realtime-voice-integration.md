...
现在的独立 smoke renderer 已移除 `@ricky0123/vad-web`、其 Silero v5 compatibility bundle 和 `ScriptProcessor` 路径，改为通过本地 Vite diagnostic entry 使用 AIRI production `workers/vad/VAD`、共享的 `resolveProductionVADConfig`、`createVADStates` 与 production `process.worklet`。Transformers 运行时固定 `allowRemoteModels=false`、`allowLocalModels=true`，模型只从仓库内 vendored 的 immutable `onnx-community/silero-vad` revision/asset 通过 loopback 提供；这使 `PRODUCTION_VAD_ALIGNMENT=YES` 的含义严格限定为 detector、model、config 和 AudioWorklet graph 已对齐 production authority，不代表 AEC Level 2/3 或 barge-in 已通过。`SOURCE_PROVEN`。
...
当前 boot repair 在 smoke 自己的 Vite server 上设置 `configFile: false` 和 `optimizeDeps.disabled: true`，只按需处理 smoke HTML、renderer 和 production VAD 模块图；正常 AIRI app 的 Vite 配置没有改变。`--preflight` 现在实际执行 `listen()`、loopback 根 HTML 获取、renderer transform、production VAD graph transform 与 server close，并显式验证没有 Web Extension plugin/entry 或 `uno.css` 要求。无媒体 preflight 已通过；这只证明 harness boot/module readiness，不是 Owner PHASE_0–4，也不是 Level 2/3 runtime PASS。Owner 仍需重新运行原有单一命令完成本机验证。`SOURCE_PROVEN` + `CURRENT_DESIGN_DECISION`。
